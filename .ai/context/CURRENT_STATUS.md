...
- 已完成 `T-S1-005`：接入真实 Emby 测试/拉曲目能力（BaseURL/用户名/密码），并将曲目来源切到 Emby 请求结果。
...
- 已完成 `T-S1-006`：新增 API 17 实机交互回归清单与结果模板，覆盖安装/启动/交互/前后台切换。
...
- 当前执行按 `TASK_QUEUE Ready` 单任务推进，当前 `Ready` 已清空（等待下一轮规划或人工回归结果）。
