...
- 决策: 点赞点击后必须有按钮状态变化反馈；本批验收以按钮 pending/success/failure/not-likeable 等可见状态变化为准。
- 决策: 发现页一级 tab 不再使用边框框选，改用高亮选中态，增大字号并尽量一排放下。
- 决策: 发现页二级 tab 也不使用边框框选，使用区别于一级 tab 的高亮样式，增大字号并保持多行清晰。
- 决策: 手动点击左侧“每日推荐”只切页面并展示推荐列表，不得替换当前播放列表或改变当前播放。
- 决策: 只有点击每日推荐列表中的歌曲时，才用当日推荐列表替换当前播放列表，并从点击歌曲开始播放。
- 决策: 冷启动/首次进入首页自动加载并播放每日推荐第一首的既有口径继续保留，除非后续用户明确改口。
- 决策: 方向盘上下曲已确认全局可用；本批不再规划该项。
- 决策: 方向盘语音按钮覆盖/语音助手、系统首页卡片、系统镜像进一步分析和高德车机调用联动全部写入 `.ai/context/SYSTEM_IMAGE_DISCUSSION_NOTES.md`，作为未来讨论项，不进入当前 Ready。

## 2026-06-08 - 首页体验与发现页紧凑重构口径（用户确认）
- 决策: 首次进入首页必须加载并播放每日推荐列表第一首；但用户手动切到任意其它播放列表后，每日推荐不得再次自动抢播。
- 决策: 左侧“每日推荐”入口点击后只展示当日推荐列表，不直接播放；用户点击推荐列表歌曲后才建立队列并播放。
- 决策: 首页右侧必须是当前播放队列的实时视图；当前歌曲变化时必须更新选中态并自动滚动，覆盖手动下一曲、自然下一曲、失败跳过、无 VIP/无权限跳过。
- 决策: `.NET` UI 没有独立 Scene 页面；Android 当前独立 Scene 左侧入口不继续作为产品入口扩展，场景能力并入发现页的一级/二级 tab 与歌单网格。
- 决策: 发现页 UI 去掉“发现歌单”标题行、“二级分类：xxx”提示行和刷新按钮；一级/二级分类切换时自动获取歌单。
- 决策: 发现页一级 tab 全量展示；二级 tab 不横向滚动，多行换行；有图分类默认两行，多余展开/收缩，无图标签默认三行，多余展开/收缩，点击后自动收缩。
- 决策: 首页歌词必须恢复，歌词直接从酷狗查；参考 `.NET` `LyricClient` / `RawLyricApi`，先 `lyrics.kugou.com/v1/search` 拿候选，再 `/download` 获取并解码 KRC/LRC。
- 决策: 当首页当前切在播放列表 tab 且正在播放时，10 秒无用户操作自动切回歌词 tab；空播放状态不处理。
- 决策: 首页播放块删除按钮前增加点赞按钮，行为复用其它页面酷狗点赞逻辑、状态记录、失败提示与脱敏观测。
- 决策: 本轮继续遵守 `MainActivity` 红线，发现页、歌词、队列同步和点赞按钮不得把协议/解析/状态机堆回入口文件。
...
- 决策: 运行中 token/session 失效时，不清空现有内容；应弹出登录窗口要求重新登录。
- 决策: 登录成功后隐藏登录弹窗，并继续当前页面或当前列表的加载/重试流程。
- 决策: 首页登录交互改为弹窗登录，不再以内嵌登录面板作为主要交互。
- 决策: 该需求需要新增脱敏观测：登录后自动加载、页面懒加载、token 失效弹窗、登录后恢复/重试均应有 PostHog/runtime/logcat 证据，不记录 token/session/cookie/完整 URL query/手机号/验证码/auth header。
...
## 2026-06-09 - API17 应用内更新安装修复决策
- 决策: 当前“包解析失败”按应用内更新安装链路缺陷处理；因同版本 APK 通过“甲壳虫 ADB 助手”安装可成功，APK 本体兼容性不是首要假设。
- 决策: 本轮采用平衡方案 B：修复 API17-safe 文件保存、完整性/预解析、安装 intent URI/MIME 和系统安装器 handoff。
- 决策: 不重做整套更新系统，不做静默安装，不改变 GitHub Releases 作为更新真源。
- 决策: 更新安装链路必须新增脱敏诊断，能区分下载损坏、文件不可读、intent/URI/MIME 不兼容、签名/版本/SDK 不兼容。
