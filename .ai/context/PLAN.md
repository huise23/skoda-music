...
## Current Stage
- Stage Name: S4 车机后台控制落地（方案1 / Legacy 稳态 + 更新能力并行）
- Scope Source: `.ai/context/SCOPE.md`（2026-04-27）
- Stage Goal: 在 API17 红线下完成“后台可控 + 浮窗可控 + 恢复可控 + 更新可控”的可实机验收闭环。
...
- 冷启动自动更新检测（节流）与设置页手动检测。
- GitHub Release 查询 + 镜像加速下载 + 安装跳转。
- 车机实机回归与证据回填。
...
- GitHub 镜像策略待确认（默认优先级、失败回退、是否允许用户自定义）。
- 更新检测周期与网络策略待确认（建议默认 24h + 手动强制检测）。
...
### W6 更新检测与分发链路
- 目标: 提供冷启动自动检测与设置手动检测，并支持 GitHub 镜像加速下载。
- 输出: API17 可用的版本检测客户端、更新 UI 入口、下载与安装触发闭环。
...
- `M-S4-UPD-007` 与 `M-S4-OBS-006` 并行推进，并在 `M-S4-VALID-004` 前完成最小闭环。
- `M-S4-UX-005` 并行存在，但当前保持 Deferred。
- 车机实机执行依赖 `M-S4-CORE-001/002/003`，并建议带上 `M-S4-OBS-006` 与 `M-S4-UPD-007` 的关键链路。
...
- 风险: GitHub API 频率限制与网络波动会导致检测失败或下载中断。
- 风险: 镜像域名可用性不稳定，需明确回退到官方下载链接。
- 假设: 允许前台服务常驻通知。
- 假设: 目标车机可授予悬浮窗权限。
- 假设: PostHog 采用“结构化事件”而非全量文本日志上报。
- 假设: 允许通过系统安装器进行 APK 覆盖安装。
...
- M5: 完成更新检测与镜像下载最小闭环（冷启动自动检测 + 设置手动检测 + 下载触发安装）。
- M6: 完成 API17 实机回归与证据回写（阶段验收里程碑）。

## Recommended Path
1. 优先推进 `M-S4-CORE-001`（模块执行），收口核心命令链路。
2. 并行推进 `M-S4-OBS-006`，先完成事件模型、API17 兼容上报层与埋点门禁。
3. 并行推进 `M-S4-UPD-007`，先落地更新检测客户端与设置页入口，再接镜像下载回退链路。
4. 并行完成 `T-S4-VAL-032`（微任务），补齐 S4 回归清单与验收模板。
5. 进入 `M-S4-CONTROL-002` 与 `M-S4-RESUME-003`，形成可上车机版本。
6. 触发 `T-S4-REG-022` 实机回归，完成 `T-S4-VAL-033` 证据回填。
