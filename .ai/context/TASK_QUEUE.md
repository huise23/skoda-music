...
## Ready
- [ ] `T-S4-AUDIO-065` EQ 卡片视觉分组与状态行重排（布局实现）
...
## Blocked
- [ ] `T-S4-AUDIO-066` EQ 状态模型接线与 UI 渲染（依赖 `T-S4-AUDIO-065`）
- [ ] `T-S4-AUDIO-067` EQ 文案与交互反馈收口（依赖 `T-S4-AUDIO-066`）
- [ ] `T-S4-AUDIO-068` EQ UI 回归与 API17 观察点补齐（依赖 `T-S4-AUDIO-067`）
...
## Queue Notes
- 本阶段主线已从“EQ 能力接线”切到“EQ 界面规划与任务化”，先规划后实现。
